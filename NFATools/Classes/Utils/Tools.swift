...
open class Tools {
    ...
    /// 密码加密串
    open class func getPassEncryptCode() -> String{
        return Const.pass_suffix
    }
    ...
    open class func validateEmpty(_ baseView : UIViewController, msgs : [String], textFields : [UITextField]) -> Bool {
        ...
        for i in 0 ..< count {
            if (textFields[i].text == nil || textFields[i].text == "") {
                //                CustomAlertView.shareInstance.showDefaultWarningAlertOnView(baseView.view, withTitle: msgs[i] + "不能为空")
                baseView.showWarningMessage(msgs[i] + "不能为空")
                return false
            }
        }
        return true
